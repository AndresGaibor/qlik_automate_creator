      ...
      <CardHeader className="border-b border-line-200 bg-app/30 pb-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-ink-400 font-mono">
            {ejecuciones.length}{" "}
            {ejecuciones.length === 1 ? "ejecución" : "ejecuciones"} registradas
          </span>
        </div>
      </CardHeader>
      <CardContent className="p-0">
        {!ejecuciones || ejecuciones.length === 0 ? (
          <div className="p-8 text-center text-sm text-ink-400">
            Esta automatización aún no ha sido ejecutada. Haz clic en «Ejecutar
            ahora» para iniciarla.
          </div>
        ) : (
